    ...
    Callback: PacketCallback + Send,
{
    ...
    pub fn pick_up_item(&mut self, item_entity_id: EntityId) -> Result<(), NotConnectedError> {
        match self.map_server_packet_version()? {
            SupportedPacketVersion::_20220406 => self.send_map_server_packet(ItemPickupRequestPacket::new(item_entity_id)),
        }
    }
